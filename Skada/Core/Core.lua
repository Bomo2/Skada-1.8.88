  ...
  -- prevents combat from starting and filters combat events for ignored creatures
  local function ShouldIgnoreCreature(guid, name)
    if not guid then return false end

    local npcID = GetCreatureId(guid)
    if npcID and npcID > 0 and ns.ignored_creatures[npcID] then
        return true
    end

    if UnitExists("target") and UnitGUID("target") == guid then
        if UnitClassification("target") == "trivial" then
            return true
        end
    end
  ...
  do
  	...
  	function Skada:ShowTooltip(win, id, label, bar, class)
  		...
  		-- Section 1: Add Spells and Targets first.
  		if P.informativetooltips then
  			add_submode_lines(md.click1, win, id, label, t, class) -- Spells
  			add_submode_lines(md.click2, win, id, label, t, class) -- Targets
  		end
  
  		-- Section 2: Add a blank line for spacing after the Targets section.
  		t:AddLine(" ")
  
  		-- Section 3: Add the main "Activity" tooltip last.
  		if md.tooltip then
  			local numLines = t:NumLines()
  			md.tooltip(win, id, label, t)
  
  			-- This adds the second gap before the "Click for..." hints.
  			if t:NumLines() ~= numLines and hasClick then
  				t:AddLine(" ")
  			end
  		end
  
  		-- Section 4: Add the "Comparison" section if it exists.
  		if P.informativetooltips then
  ...
  function Skada:SmartStop(set)
    if P.smartstop then
        local delay = P.smartwait or 3
        Skada:ScheduleTimer(function()
            if Skada.current == set and not set.stopped then
                Skada:StopSegment(L["Segment Stopped."])
                Skada:SetModes()
            end
        end, delay)
    else
  ...
  function Skada:PLAYER_REGEN_ENABLED()
  	if Skada.current and Skada.current.gotboss == 36597 then
  		local is_final_phase = false
  		for i = 1, 5 do
  			local unit = "boss"..i
  			if UnitExists(unit) and GetCreatureId(UnitGUID(unit)) == 36597 then
  				if (UnitHealth(unit) / UnitHealthMax(unit)) <= 0.11 then
  					is_final_phase = true
  				end
  				break
  			end
  ...
  function Skada:PLAYER_REGEN_DISABLED()
  	if self.disabled or self.current then return end
  	if P.tentativecombatstart then
  		if UnitExists("target") and ShouldIgnoreCreature(UnitGUID("target"), UnitName("target")) then
  			return 
  		end
  ...
  do
  	...
  	if t.dstGUID and ShouldIgnoreCreature(t.dstGUID, t.dstName) then
  		return
  	end
      ...
      if not self.current and trigger_events[t.event] and t.srcName and t.dstName and t.srcGUID ~= t.dstGUID then
          ...
          -- This block is the root of the "tentative combat" system. By placing our check here,
          -- we can stop the tentative counter from ever starting on an ignored creature.
          if ShouldIgnoreCreature(t.dstGUID, t.dstName) then
              return
          end
